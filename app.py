...
site = st.sidebar.selectbox(
    ...
    ["CIP Hatalageri","JSW Tuljapur","Blupine Sagapara","Kalavad GJ",
     "Kalavad_PH2","AMP_Energy","Wanki","CleanMax Motadevaliya","Ayana Amerli"]
)
...
def load_reference(site):
    try:
        ...
        st.error("Reference Excel file missing!")
        st.stop()
...
def process_turbine(t):
    ...
    # MEASUREMENT
    measurement_flag = len(
        df_t[(df_t[wind_col] < 4) & (df_t[power_col] > 0.2 * RATED_POWER)]
    ) > 10

    high_perf_flag = avg_dev > 8

    return df_t, merged, avg_dev, stall_flag, stall_bins, availability, std_dev, derating_flag, measurement_flag, high_perf_flag, active_flags
...
for t in df["Name"].unique():
    ...
    df_t, merged, avg_dev, stall_flag, stall_bins, availability, std_dev, derating_flag, measurement_flag, high_perf_flag, active_flags = res
    ...
    if stall_flag:
        ...
        if derating_flag:
            comment += "Cause: DERATING ACTIVE\n"
        else:
            comment += "Cause: Aerodynamic / IPC / Yaw issue\n"
    ...
    if avg_dev > 8:
        ...
        comment += "Measurement / Sensor / IPC issue\n"
